...
#
#brighten the image by 10, but wont get brightened if pixel reaches
#255

def bIntensify(pixel):
    pixel_1d = {(pixel[0]+10), 255}
    pixel_2d = {(pixel[1]+10), 255}
    pixel_3d = {(pixel[2]+10), 255}
    return (( min(pixel_1d), min(pixel_2d), min(pixel_3d)))

             
#darken the image by 10, but won't get darken if pixel reaches 0
def dIntensify(pixel):
    pixel1d = {(pixel[0]-10), 0}
    pixel2d = {(pixel[1]-10), 0}
    pixel3d = {(pixel[2]-10), 0}
    return (( max(pixel1d), max(pixel2d), max(pixel3d)))
...
def brtn(image_surf):
    ...
    for x in range(rows):
        for y in range(cols):
            pixels3d[x,y] = bIntensify(pixels3d[x,y])
...
def drkn(image_surf):
    ...
    for x in range(rows):
        for y in range(cols):
            pixels3d[x,y] = dIntensify(pixels3d[x,y])
